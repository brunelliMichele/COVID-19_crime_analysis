...
from utils import (
    load_crime_data, load_shapes, calc_variation,
...
st.markdown("Crime variation (%) from pre-COVID baseline (2014-2019) to 2020")
...
# load data for geo level
shapes = load_shapes(geo_level)
crimes_filtered = filter_crime_by_level(crimes, geo_level)

# var calc
if selected_crime:
    var_df = calc_variation(crimes, selected_crime)
else:
...
if n_valid == 0:
    st.error("No avilable data for this type of crime")
else:
    # map
